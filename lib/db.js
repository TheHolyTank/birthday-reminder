...
export async function ensureSchema() {
  ...
  // matches the original single global schedule), 0 = day of. Hour is a UTC
  // hour (0-23); default 17 matches vercel.json's original 17:30 UTC.
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS reminder_offset_days SMALLINT NOT NULL DEFAULT 1;`;
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS reminder_hour_utc SMALLINT NOT NULL DEFAULT 17;`;
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_users_reminder_hour_utc ON users (reminder_hour_utc);`;
  await sql`
